...
5. 在函式選單執行一次 `authorizeCalendar`，確認授權 Google Calendar 存取。
...
信件內文包含完整預約明細：預約與入住時段、飼主與緊急聯絡資料、各房型與房間數量、伙食方案、費用拆分、每隻貓咪的飲食／健康／照護資料、其他補充及 Google Sheet 連結。Email 寄送失敗不會影響預約寫入。
...
2. 確認按鈕由「正在登記預約」改為「預約資料已送出」，頁面自動捲動至訂金匯款帳戶，並顯示預約編號與住宿須知圖片；「複製帳號」應複製純數字帳號 `21072000223851`。
...
5. 確認每間基本房價包含 1 隻貓，多出的貓咪每隻每晚加收 NT$200；7–13 晚住宿費為 95 折、14–29 晚為 9 折、滿 30 晚（視為一個月）為 7 折，且超時安親費仍以未折扣的單晚房價計算。
6. 分別確認乾飼料／套組依「每隻貓 × 加購天數」計價，罐頭則依整筆預約的總罐數計價、不再乘以貓咪數量，且加購天數不會超過住宿晚數。
7. 確認緊急聯絡人姓名、電話與關係均留空時仍可送出，試算表、Email 與 Calendar 以空白或「未填」呈現；並確認每個指定信箱都收到一封含完整預約、各房型數量、伙食數量與貓咪資料的新預約通知，混合房型不得漏掉其中一種。
8. 確認「住宿預約」第 25 欄顯示「已寄送」、第 35 欄正確記錄房間數量、第 36 欄顯示「已建立」，且第 37 欄有 Calendar 行程 ID。
9. 確認貓家日子的 Google Calendar 只有一筆相同預約編號的行程，日期、入住／退宿時間、標題與說明正確；混合房型應完整列出兩種房型與數量。
10. 確認 Apps Script 回應無法讀取時，頁面最晚會在逾時保護啟動後結束「正在登記預約」狀態，不會永久卡住。
11. 確認入住時間早於 10:00、退宿時間晚於 20:30，以及退宿時間與 15:00 前後區間不一致時，都無法送出。
12. 測試完成後，刪除兩張表內相同預約編號的測試資料，並刪除相同預約編號的 Calendar 測試行程。
